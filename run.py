...
FALLBACK_PACKAGE_TO_IMPORT_NAME = {"Pillow": "PIL", "curl-cffi": "curl_cffi"}
...
def get_package_to_import_mapping():
    ...
    try:
        ...
        debug_print(
            f"Warning: Could not build package-to-import mapping: {e}", is_error=True
        )
...
def check_python_version():
    ...
    if version_info.major < 3 or (version_info.major == 3 and version_info.minor < 12):
        debug_print(
            f"Error: Python 3.12 or higher is required. You are using Python {platform.python_version()}",
            is_error=True,
        )
        sys.exit(1)
...
def check_required_files(script_dir):
    ...
    if not os.path.exists(interactive_script) and sys.stdin.isatty():
        debug_print(
            "Warning: _matterport_interactive.py not found. Interactive mode will not be available.",
            is_error=True,
        )
...
def setup_venv(script_dir):
    ...
    if os.path.exists(venv_dir) and os.path.exists(
        os.path.join(venv_dir, "Scripts" if sys.platform == "win32" else "bin")
    ):
        return venv_dir
...
def run_in_venv(script_dir, venv_dir):
    ...
    debug_print(
        f"Restarting in virtual environment python executable: {python_executable} ..."
    )
    try:
...
def is_module_installed(module_name, package_to_import_mapping):
    ...
    if (
        "-" in module_name
        and importlib.util.find_spec(module_name.lower().replace("-", "_")) is not None
    ):
        return True
...
def check_and_install_modules(script_dir, venv_dir):
    ...
    if not os.path.exists(requirements_file):
        debug_print(
            f"Error: requirements.txt not found in {script_dir} if you are sure you have all requirements installed you bypass this script and call matterport-dl.py directly",
            is_error=True,
        )
        sys.exit(1)
                ...
                try:
                    ...
                    if required_version.startswith(">="):
                        ...
                        if parse_version(installed_version) < parse_version(
                            ...
                            debug_print(
                                f"Module {module_name} version {installed_version} is older than required {required_version}"
                            )
                            missing_modules.append(module_name)
                    elif installed_version != required_version:
                        debug_print(
                            f"Module {module_name} version {installed_version} doesn't match required {required_version}"
                        )
                        missing_modules.append(module_name)
                    ...
                    if required_version and "==" in required_version:
                        debug_print(
                            f"Module {module_name} metadata not found, marking for installation"
                        )
                        missing_modules.append(module_name)
        except Exception as e:
    ...
    if missing_modules:
        ...
        debug_print(
            f"Installing/upgrading the following modules: {', '.join(missing_modules)}"
        )
        try:
...
def run_matterport_dl(script_dir):
    ...
    if matterdl is not None:
        ...
        debug_print(
            f"Error running matterport-dl.py directly, trying subprocess...",
            is_error=True,
        )
...
def check_external_python_version():
    ...
    if EXTERNAL_PY_VERSION_VAR in os.environ:
        ...
        debug_print(
            f"External Python version: {parse_version(external_version)} vs virtual env version: {parse_version(current_version)}"
        )

        if parse_version(external_version) > parse_version(current_version):
            debug_print(
                "Warning: External Python version has been upgraded since this virtual environment was created.",
                is_error=True,
            )
            debug_print(
                "To use the newer Python version, delete the 'venv' folder next to run.py and run this script again.",
                is_error=True,
            )
            debug_print(
                f"External version: {external_version}, Current virtual env version: {current_version}",
                is_error=True,
            )
