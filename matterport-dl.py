...
import warnings; warnings.filterwarnings("ignore", category=UserWarning, module="curl_cffi")
...
MAX_CONCURRENT_REQUESTS = (
    20  # cffi will make sure no more than this many curl workers are used at once
)
...
BASE_MODEL_ID = (
    ""  # normally this is the model id we are downloading unless defurnished
)
...
AccessKeyType = Enum(
    ...
    [
        "LeaveKeyAlone",
        "PrimaryKey",
        "MAIN_PAGE_GENERIC_KEY",
        "MAIN_PAGE_DAM_50K",
        "FILES2_BASE_URL_KEY",
        "FILES3_TEMPLATE_KEY",
        "SWEEP_KEY",
        "GRAPH_MODEL_VIEW_PREFETCH",
    ],
)  # sweep key primarily used for defurnished, GRAPH_MODEL_VIEW_PREFETCH is only used for attachments
...
def consoleDebugLog(msg: str, loglevel=logging.INFO, forceDebugOn=False):
    ...
    if not CLA.getCommandLineArg(CommandLineArg.CONSOLE_LOG) and (
        forceDebugOn or CLA.getCommandLineArg(CommandLineArg.DEBUG)
    ):
        print(msg)
...
async def downloadDAM(accessurl, uuid):
    ...
    await downloadFile(
        "UUID_DAM50K",
        True,
        accessurl.format(filename=f"{uuid}_50k.dam"),
        f"..{os.path.sep}{uuid}_50k.dam",
        key_type=AccessKeyType.FILES3_TEMPLATE_KEY,
    )
    shutil.copy(
        damSrcFile, f"{uuid}_50k.dam"
    )  # so the url here has the ~ in it but the primary dir is the parent sitl lwe will store it both places
    cur_file = ""
    try:
        for i in range(
            1000
        ):  # basically download until on first failure and assume that is all of them, maybe we should be going to on first 404 or osmething:)
            ...
            await downloadFile(
                ...
                True,
                cur_file,
                f"{uuid}_50k_texture_jpg_high/{uuid}_50k_{i:03d}.jpg",
            )
            cur_file = accessurl.format(
            ...
            await downloadFile(
                ...
                True,
                cur_file,
                f"{uuid}_50k_texture_jpg_low/{uuid}_50k_{i:03d}.jpg",
            )
...
async def downloadSweeps(accessurl: str, sweeps: list[str]):
    ...
    for sweep in sweeps:
        ...
        for variant in getVariants():
            toDownload.append(
                AsyncDownloadItem(
                    "MODEL_SWEEPS",
                    True,
                    accessurl.format(filename=f"tiles/{sweep}/{variant}")
                    + "&imageopt=1",
        ...
        for face_num in range(6):
            face_file = f"512_face{face_num}_0_0.jpg"
            toDownload.append(
                AsyncDownloadItem(
                    "MODEL_SWEEPS",
                    True,
                    accessurl.format(filename=f"tiles/{sweep}/{face_file}")
                    + "&imageopt=1",
                    f"tiles/{sweep}/{face_file}",
                    key_type=forceKey,
                )
            )
...
async def downloadFileWithJSONPostAndGetText(
    type, shouldExist, url, file, post_json_str, descriptor, always_download=False
):
    if not CLA.getCommandLineArg(CommandLineArg.TILDE):
        file = file.replace("~", "_")

    await downloadFileWithJSONPost(
        type, shouldExist, url, file, post_json_str, descriptor, always_download
    )
...
async def downloadFileWithJSONPost(
    type, shouldExist, url, file, post_json_str, descriptor, always_download=False
):
    global OUR_SESSION
    if not CLA.getCommandLineArg(CommandLineArg.TILDE):
    ...
    if not CLA.getCommandLineArg(CommandLineArg.DOWNLOAD) or (
        os.path.exists(file) and not always_download
    ):  # skip already downloaded files except forced downloads
        logUrlDownloadSkipped(type, file, url, descriptor)
        return

    reqId = logUrlDownloadStart(
        type, file, url, descriptor, shouldExist, key_type=AccessKeyType.PrimaryKey
    )
    try:
        resp: requests.Response = await OUR_SESSION.request(
            url=url,
            method="POST",
            headers={"Content-Type": "application/json"},
            data=bytes(post_json_str, "utf-8"),
        )
        ...
        raise Exception(
            f"Request error for url: {url} ({type}) that would output to: {file} of: {ex}"
        ) from ex  # str(ex) only is really doing the message not the entire cert  but we want the general error msg
...
async def GetTextOnlyRequest(type, shouldExist, url, post_data=None) -> str:
    ...
    result = await downloadFileAndGetText(
        type, shouldExist, url, useTmpFileName, post_data
    )
...
async def downloadFileAndGetText(
    type,
    shouldExist,
    url,
    file,
    ...
    key_type: AccessKeyType = AccessKeyType.PrimaryKey,
):
    if not CLA.getCommandLineArg(CommandLineArg.TILDE):
        file = file.replace("~", "_")

    await downloadFile(
        type, shouldExist, url, file, post_data, always_download, key_type
    )
    if not os.path.exists(file):
        return ""
    else:
...
async def downloadFile(
    ...
    async with MAX_TASKS_SEMAPHORE:
        ...
        if not CLA.getCommandLineArg(CommandLineArg.DOWNLOAD) or (
            os.path.exists(file) and not always_download
        ):  # skip already downloaded files except always download ones which are genreally ones that may contain keys?
        ...
        try:
            ...
            if "?t=" in url and "Error 404" not in f"{err}":
                if (
                    False
                ):  # disable brute forcing at a minimum probably shouldnt do getallkeys just primary
                    for key in KeyHandler.GetAllKeys():
                        ...
                        try:
                            ...
                            async with aiofiles.open(file, "wb") as f:
                                await f.write(response.content)
                            logUrlDownloadFinish(
                                type, file, url2, "", shouldExist, reqId
                            )
                            return
                        except Exception as err2:
                            logUrlDownloadFinish(
                                type, file, url2, "", shouldExist, reqId, err2, True
                            )
                            pass
            logUrlDownloadFinish(type, file, url, "", shouldExist, reqId, err)
            raise Exception(
                f"Request error for url: {url} ({type}) that would output to: {file} of: {err}"
            ) from err
...
async def downloadGraphModels(pageid):
    ...
    for key in GRAPH_DATA_REQ:
        ...
        text = await downloadFileAndGetText(
            "GRAPH_MODEL",
            True,
            f"https://my.{BASE_MATTERPORT_DOMAIN}/api/mp/models/graph{req_url}",
            file_path,
            always_download=CLA.getCommandLineArg(CommandLineArg.REFRESH_KEY_FILES)
            and CLA.getCommandLineArg(CommandLineArg.ALWAYS_DOWNLOAD_GRAPH_REQS),
        )
        ...
        if key == "GetModelViewPrefetch" and not MODEL_IS_DEFURNISHED:
            ...
            if len(prefetchKeys) > 0:
                KeyHandler.SetAccessKey(
                    AccessKeyType.GRAPH_MODEL_VIEW_PREFETCH, prefetchKeys[0]
                )
        ...
        if CLA.getCommandLineArg(CommandLineArg.MANUAL_HOST_REPLACEMENT):
            text = text.replace(
                f"https://cdn-2.{BASE_MATTERPORT_DOMAIN}", "http://127.0.0.1:8080"
            )  # without the localhost it seems like it may try to do diff
            text = text.replace(
                f"https://cdn-1.{BASE_MATTERPORT_DOMAIN}", "http://127.0.0.1:8080"
            )  # without the localhost it seems like it may try to do diff
        text = validUntilFix(text)

        async with aiofiles.open(
            getModifiedName(file_path), "w", encoding="UTF-8"
        ) as f:
            await f.write(text)
...
ProgressType = Enum(
    "ProgressType",
    ["Request", "Success", "Skipped", "Failed404", "Failed403", "FailedUnknown"],
)
...
def logUrlDownloadFinish(
    ...
    if error:
        if altUrlExists:
            ...
            prefix = "aFailure"
            PROGRESS.Increment(
                ProgressType.Failed403
                if "Error 403" in error
                else (
                    ProgressType.Failed404
                    if "Error 404" in error
                    else ProgressType.FailedUnknown
                )
            )
    else:
    ...
    _logUrlDownload(
        ...
        requestID,
        error,
    )  # not sure if should lower log elve for shouldExist  false
...
def logUrlDownloadStart(
    type, localTarget, url, additionalParams, shouldExist, key_type
):
    global PROGRESS
...
def _logUrlDownload(
    ...
    requestID,
    optionalResult=None,
    key_type=AccessKeyType.PrimaryKey,
):
    global CLA
    ...
    logging.log(
        logLevel,
        f"{logPrefix} REQ for {type} {requestID}: should exist: {shouldExist} {optionalResult} File: {localTarget} at url: {url} {key_type} {additionalParams}",
    )
...
def extractJSDict(forWhat: str, str: str):
    ...
    if startPos == -1:
        raise Exception(
            f"Unable to extract JS dictionary for: {forWhat} from the JS string: {str} can't find first {{"
        )
    endPos = str.rfind("}")
    if endPos == -1:
        raise Exception(
            f"Unable to extract JS dictionary for: {forWhat} from the JS string: {str} can't find last }}"
        )
...
async def downloadAssets(base, base_page_text):
    ...
    language_codes = [
        ...
        "ru",
        "es",
    ]  # these are the only language codes that seem to succeed if a model works with one other than this please file a bug report and let us know, these are hardcoded into showcase.js file

    font_files = [
        "ibm-plex-sans-100",
        "ibm-plex-sans-100italic",
        "ibm-plex-sans-200",
        "ibm-plex-sans-200italic",
        "ibm-plex-sans-300",
        "ibm-plex-sans-300italic",
        "ibm-plex-sans-500",
        "ibm-plex-sans-500italic",
        "ibm-plex-sans-600",
        "ibm-plex-sans-600italic",
        "ibm-plex-sans-700",
        "ibm-plex-sans-700italic",
    ...
    image_files = [
        "360_placement_pin_mask",
        "chrome",
        "Desktop-help-play-button.svg",
        "Desktop-help-spacebar",
        "edge",
        "escape",
        "exterior",
        "exterior_hover",
        ...
        "interior",
        "interior_hover",
        "matterport-logo-light.svg",
        "matterport-logo.svg",
        "mattertag-disc-128-free.v1",
        "mobile-help-play-button.svg",
        "nav_help_360",
        "nav_help_click_inside",
        "nav_help_gesture_drag",
        "nav_help_gesture_drag_two_finger",
        "nav_help_gesture_pinch",
        "nav_help_gesture_position",
        "nav_help_gesture_position_two_finger",
        "nav_help_gesture_tap",
        "nav_help_inside_key",
        "nav_help_keyboard_all",
        "nav_help_keyboard_left_right",
        "nav_help_keyboard_up_down",
        "nav_help_mouse_click",
        "nav_help_mouse_ctrl_click",
        "nav_help_mouse_drag_left",
        "nav_help_mouse_drag_right",
        "nav_help_mouse_position_left",
        "nav_help_mouse_position_right",
        "nav_help_mouse_zoom",
        "nav_help_tap_inside",
        "nav_help_zoom_keys",
        "NoteColor",
        ...
        "safari",
        "scope.svg",
        "showcase-password-background.jpg",
        "surface_grid_planar_256",
        "vert_arrows",
        "headset-quest-2",
        "tagColor",
        "matterport-app-icon.svg",
    ]
    ...
    assets = [
        ...
        "images/matterport-logo-white-r.svg",
        "css/packages-nova-ui.css",
        "css/scene.css",
        "css/unsupported_browser.css",
        "cursors/grab.png",
        "cursors/grabbing.png",
        "cursors/zoom-in.png",
        "cursors/zoom-out.png",
        "locale/strings.json",
        "css/ws-blur.css",
        "css/core.css",
        "css/late.css",
    ...
    base_page_js_loads = re.findall(
        r"script\s+(?:defer\s+)?src=[\"']([^\"']+[.]js)[\"']",
        base_page_text,
        flags=re.IGNORECASE,
    )
    ...
    for js in base_page_js_loads:
        ...
        if "://" in js:
            consoleDebugLog(
                f"Skipping {js} should be the three.js file as the only non-relative one"
            )
        # if "://" not in js:
        # file = base + js
        if file in assets:
    ...
    if showcase_runtime_filename is None:
        raise Exception(
            "In all js files found on the page could not find any that have showcase and runtime in the filename for the showcase runtime js file"
        )
    if react_vendor_filename is None:
        raise Exception(
            "In all js files found on the page could not find any that have vendors-react in the filename for the react vendor js file"
        )
    await downloadFile(
        "STATIC_ASSET",
        True,
        f"{base}images/favicons/favicon.ico",
        "images/favicons/favicon.ico",
    )  # mainly to avoid the 404, always matterport.com
    showcase_cont = await downloadFileAndGetText(
        typeDict[showcase_runtime_filename],
        True,
        base + showcase_runtime_filename,
    ...
    if match is None:
        raise Exception(
            "Unable to extract js files and css files from showcase runtime js file"
        )
    groupDict = match.groupdict()
    jsNamedDict = extractJSDict(
        "showcase-runtime.js: namedJSFiles", groupDict["namedJSFiles"]
    )

    try:
        jsKeyDict = extractJSDict(
            "showcase-runtime.js: JSFileToKey", groupDict["JSFileToKey"]
        )
    except Exception:
        jsKeyDict = {}

    try:
        cssNamedDict = extractJSDict(
            "showcase-runtime.js: namedCSSFiles", groupDict["namedCSSFiles"]
        )
    except Exception:
        cssNamedDict = {}

    try:
        cssKeyDict = extractJSDict(
            "showcase-runtime.js: CSSFileToKey", groupDict["CSSFileToKey"]
        )
    except Exception:
        cssKeyDict = {}

    # Logic for when the hash key dictionary is found (Old Format)
    if jsKeyDict:
        for number, key in jsKeyDict.items():
            ...
            assets.append(file)
    # Logic for when the hash key dictionary is MISSING (New Format)
    elif jsNamedDict:
    ...
    found_chunks = set(re.findall(r"\.e\(([0-9]{2,5})\)", showcase_cont))
    mapping_matches = re.findall(r"([0-9]{2,5}):\"", showcase_cont)
    found_chunks.update(mapping_matches)

    for chunk_id in found_chunks:
        file = f"js/{chunk_id}.js"
        if file not in typeDict:
            typeDict[file] = "SHOWCASE_DISCOVERED_JS"
    ...
    if found_chunks:
        consoleLog(
            f"Scraped {len(found_chunks)} additional JS chunks from runtime.",
            logging.INFO,
        )
    ...
    for asset in assets:
        ...
        shouldExist = True
        toDownload.append(
            AsyncDownloadItem(type, shouldExist, f"{base}{asset}", local_file)
        )
    await AsyncArrayDownload(toDownload)

    discovered_this_round = True
    while discovered_this_round:
            ...
            for name in files:
                if name.endswith((".js", ".json")):
                    ...
                    try:
                        ...
                        # Focus only on explicit chunk calls and specific path patterns
                        found_ids = re.findall(r"\.e\(([0-9]{2,5})\)", content)
                        found_ids += re.findall(r"js/([0-9]{2,5})\.js", content)

                        # Only grab quoted IDs if they look like they belong to the current manifest
                        # This avoids grabbing the 3,000+ IDs in the runtime file that aren't for this tour
                        found_ids += re.findall(r"\"([0-9]{3,5})\"", content)

                        for eid in set(found_ids):
                            efile = f"js/{eid}.js"
                            if efile not in typeDict:
                                typeDict[efile] = "SHOWCASE_DISCOVERED_JS"
                                new_items_to_download.append(
                                    AsyncDownloadItem(
                                        "SHOWCASE_DISCOVERED_JS",
                                        True,
                                        f"{base}{efile}",
                                        efile,
                                    )
        ...
        if new_items_to_download:
            consoleLog(
                f"Deep scan discovered {len(new_items_to_download)} new nested dependencies. Downloading..."
            )
            await AsyncArrayDownload(new_items_to_download)
    ...
    user_ranges_str = CLA.enum_ranges

    if user_ranges_str:
        consoleLog(f"Starting targeted chunk enumeration with: {user_ranges_str}")

        try:
            ranges = parse_range_string(user_ranges_str)
                ...
                for i in r:
                    ...
                    if efile not in typeDict:
                        potential_chunks.append(
                            AsyncDownloadItem(
                                "SHOWCASE_DISCOVERED_JS", True, f"{base}{efile}", efile
                            )
                        )

            BATCH_SIZE = 250
            for i in range(0, len(potential_chunks), BATCH_SIZE):
                batch = potential_chunks[i : i + BATCH_SIZE]
                await AsyncArrayDownload(batch)
        except Exception as e:
            consoleLog(f"Error in enumeration: {e}")
    else:
        consoleLog("Skipping JS enumeration (no ranges provided).")
    ...
    if react_vendor_filename and os.path.exists(react_vendor_filename):
        ...
        reactCont = reactCont.replace(
            "(t.src=s.src)", '(t.src=""+(t.src??s.src))'
        )  # hacky but in certain conditions react will try to reset the source on something after it loads to re-trigger the load event but this breaks jsnetproxy.  This allows the same triggering but uses the existing source if it exists. https://github.com/facebook/react/blob/37906d4dfbe80d71f312f7347bb9ddb930484d28/packages/react-dom-bindings/src/client/ReactFiberConfigDOM.js#L744. Right now this seems to only happens on embedded attachments.
        with open(getModifiedName(react_vendor_filename), "w", encoding="UTF-8") as f:
            f.write(reactCont)
    toDownload.clear()

    if MAIN_SHOWCASE_FILENAME and os.path.exists(MAIN_SHOWCASE_FILENAME):
        with open(MAIN_SHOWCASE_FILENAME, "r", encoding="utf-8", errors="ignore") as f:
            showcase_js_cont = f.read()

        patched_js = showcase_js_cont.replace(
            '"/api/mp/"', '"http://127.0.0.1:8080/api/mp/"'
        )
        patched_js = patched_js.replace(
            'endpoint:"/api/mp/models/graph"',
            'endpoint:"http://127.0.0.1:8080/api/mp/models/graph"',
        )

        with open(getModifiedName(MAIN_SHOWCASE_FILENAME), "w", encoding="utf-8") as f:
            f.write(patched_js)
        consoleLog(f"Patched {MAIN_SHOWCASE_FILENAME} to fix API hostnames.")
...
async def downloadWebglVendors(base_page_text):
    ...
    if threeMin is None:
        raise Exception(
            f"Unable to extract the 3d js file name from the page, regex did not match: {regex}"
        )
    ...
    for script in webglVendors:
        ...
        toDownload.append(
            AsyncDownloadItem("WEBGL_FILE", False, url, urlparse(url).path[1:])
        )
    await AsyncArrayDownload(toDownload)
...
class ExceptionWhatExceptionTaskGroup(asyncio.TaskGroup):
    ...
    async def __aexit__(
        self, et, exc, tb
    ):  # at end of block it would throw any exceptions
        try:
...
async def downloadFixedAPIInfo(pageid):
    ...
    assets = [
        f"api/v1/jsonstore/model/highlights/{pageid}",
        f"api/v1/jsonstore/model/Labels/{pageid}",
        f"api/v1/jsonstore/model/mattertags/{pageid}",
        f"api/v1/jsonstore/model/measurements/{pageid}",
        f"api/v1/player/models/{pageid}/thumb?width=1707&dpr=1.5&disable=upscale",
        f"api/v2/models/{pageid}/sweeps",
        "api/v2/users/current",
        f"api/player/models/{pageid}/files",
        f"api/v1/jsonstore/model/trims/{pageid}",
        "api/v1/plugins?manifest=true",
        f"api/v1/jsonstore/model/plugins/{pageid}",
    ]
    toDownload: list[AsyncDownloadItem] = []
    for asset in assets:
        local_file = asset
        ...
        toDownload.append(
            AsyncDownloadItem(
                "MODEL_INFO",
                True,
                f"https://my.{BASE_MATTERPORT_DOMAIN}/{asset}",
                local_file,
            )
        )
    await AsyncArrayDownload(toDownload)
...
async def downloadInfo(pageid):
    ...
    modelJson = await downloadFileAndGetText(
        "MODEL_INFO",
        True,
        f"https://my.{BASE_MATTERPORT_DOMAIN}/{pageJsonFile}",
        f"{pageJsonFile}/index.html",
        always_download=CLA.getCommandLineArg(CommandLineArg.REFRESH_KEY_FILES),
    )
    KeyHandler.SaveKeysFromText("ApiV1PlayerModelsJson", modelJson)
    for i in range(1, 4):  # file to url mapping
        fileText = await downloadFileAndGetText(
            "FILE_TO_URL_JSON",
            True,
            f"https://my.{BASE_MATTERPORT_DOMAIN}/api/player/models/{pageid}/files?type={i}",
            f"api/player/models/{pageid}/files_type{i}",
            always_download=CLA.getCommandLineArg(CommandLineArg.REFRESH_KEY_FILES),
        )  # may have keys
        KeyHandler.SaveKeysFromText(
            f"FilesType{i}", fileText
        )  # used to be more elegant but now we can just gobble all the keys
...
async def downloadPlugins(pageid):
    ...
    for plugin in pluginJson:
        ...
        await downloadFile(
            "PLUGIN",
            True,
            f"https://static.{BASE_MATTERPORT_DOMAIN}/{plugPath}",
            plugPath,
        )
    for plugin in ["measurement", "mattertag-content-editor", "minimap", "compass"]:
        if plugin == "compass":
            path = "showcase-sdk/plugins/published/compass/1.0.14/plugin.json"
            await downloadFile(
                "PLUGIN", True, f"https://static.matterport.com/{path}", path
            )
...
async def downloadAttachments():
    ...
    try:
        with open(
            "api/mp/models/graph_GetModelViewPrefetch.json", "r", encoding="UTF-8"
        ) as f:
            graphModelSnapshotsJson = json.loads(f.read())
        toDownload: list[AsyncDownloadItem] = []

        # Iterate through tags to find attachments
                ...
                for attachment in mattertag["fileAttachments"]:
                    toDownload.append(
                        AsyncDownloadItem(
                            ...
                            True,
                            attachment["url"],
                            urlparse(attachment["url"]).path[1:],
                            key_type=AccessKeyType.LeaveKeyAlone,
                        )
                    )

        if len(toDownload) == 0:
            consoleLog("No embedded attachments found in this model.", logging.INFO)
            return
        ...
        logging.exception(
            "Unable to open graph model for prefetch output and download the embedded attachments..."
        )
        return
...
async def downloadPics(pageid):
    ...
    for image in modeldata["images"]:
        toDownload.append(
            AsyncDownloadItem(
                "MODEL_IMAGES", True, image["src"], urlparse(image["src"]).path[1:]
            )
        )  # want want to use signed_src or download_url?
    await AsyncArrayDownload(toDownload)
...
async def downloadMainAssets(pageid, accessurl):
    ...
    sweepUUIDs: list[str] = []
    if (
        MODEL_IS_DEFURNISHED
    ):  # technically we could use this for all, and this data is in the prefetch embedded as well
        with open(
            "api/mp/models/graph_GetShowcaseSweeps.json", "r", encoding="UTF-8"
        ) as f:
            ...
            accessurl = base_node["locations"][0]["pano"]["skyboxes"][0][
                "tileUrlTemplate"
            ]
            ...
            accessurl = (
                accessurl
                + "{filename}?t=2-796d5d010d7183bce7f0999701973d8b05b2df8f-1735673498-0"
            )  # access key here doesnt matter as we will be replacing it
            makeDirs(sweepDir)
            os.chdir(sweepDir)
    else:
        ...
        with open(
            f"api/v1/player/models/{pageid}/index.html", "r", encoding="UTF-8"
        ) as f:
            modeldata = json.load(f)
    ...
    await downloadSweeps(
        accessurl, sweepUUIDs
    )  # sweeps are generally the biggest thing minus a few modles that have massive 3d detail items
...
def patchShowcase():
    ...
    j = j.replace(
        f'e.get("https://static.{BASE_MATTERPORT_DOMAIN}/geoip/",{{responseType:"json",priority:n.ru.LOW}})',
...
def RemoteDomainsReplace(str: str):
    global BASE_MATTERPORT_DOMAIN
    domReplace = [
        f"static.{BASE_MATTERPORT_DOMAIN}",
        f"cdn-2.{BASE_MATTERPORT_DOMAIN}",
        f"cdn-1.{BASE_MATTERPORT_DOMAIN}",
        "mp-app-prod.global.ssl.fastly.net",
        f"events.{BASE_MATTERPORT_DOMAIN}",
    ]
...
async def downloadCapture(pageid):
    ...
    base_page_text = ""
    try:
        base_page_text: str = await downloadFileAndGetText(
            "MAIN",
            True,
            url,
            "index.html",
        ...
        if "certificate verify failed" in str(
            error
        ) or "SSL certificate problem" in str(error):
            raise TypeError(
                f"Error: {str(error)}. Have you tried running the Install Certificates.command (or similar) file in the python folder to install the normal root certs? If you are using a proxy without that cert installed in the python certificate library you may need to use: --no-verify-ssl"
            ) from error
        else:
    ...
    base_page_deunicode = base_page_text.encode("utf-8", errors="ignore").decode(
        "unicode-escape"
    )  # some non-english matterport pages have unicode escapes for even the generic url chars
    if CLA.getCommandLineArg(CommandLineArg.DEBUG):
        DebugSaveFile("base_page_deunicode.html", base_page_deunicode)  # noqa: E701
    match = re.search(
        r'"(?P<baseurl>https://cdn-\d*\.matterport(?:vr)?\.(?:com|cn)/models/[a-z0-9\-_/.]*/)(?:[{}0-9a-z_/<>.~]+)(?P<defaultAccessKey>\?t=.*?)"',
        base_page_deunicode,
    )  # the ~/ optional is mostly for defurnished secondary models
    # matterportvr.cn
    if match:
        groupDict = match.groupdict()
        accessurl = (
            f"{groupDict['baseurl']}~/{{filename}}{groupDict['defaultAccessKey']}"
        )

    else:
        raise Exception(
            f"Can't find urls, try the main page: {url} in a browser to make sure it loads the model correctly"
        )
    ...
    if not MODEL_IS_DEFURNISHED:
        ...
        file_type_content = await downloadFileAndGetText(
            "MAIN",
            True,
            f"https://my.{BASE_MATTERPORT_DOMAIN}/api/player/models/{pageid}/files?type=3",
            f"api/player/models/{pageid}/files_type3",
            always_download=CLA.getCommandLineArg(CommandLineArg.REFRESH_KEY_FILES),
        )  # get a valid access key, there are a few but this is a common client used one, this also makes sure it is fresh, note we would download this one later as well but we want this key early
        KeyHandler.SetAccessKey(
            AccessKeyType.FILES3_TEMPLATE_KEY,
            KeyHandler.GetKeysFromStr(file_type_content)[0],
        )

    consoleLog(
        "Downloading graph model data..."
    )  # need the details one for advanced download
    ...
    injectedjs = (
        'if (!window.location.search.startsWith("?m='
        + pageid
        + '")) { document.location.search = "?m='
        + pageid
        + '"; };window._NoTilde='
        + ("false" if CLA.getCommandLineArg(CommandLineArg.TILDE) else "true")
        + ";window._ProxyBase="
        + forcedProxyBase
        + ";"
    )
    ...
    if CLA.getCommandLineArg(CommandLineArg.MANUAL_HOST_REPLACEMENT):
        ...
        content = re.sub(
            r"(?P<preDomain>src\s*=\s*['" '"])https?://[^/"' "']+/",
            r"\g<preDomain>",
            content,
            flags=re.IGNORECASE,
        )  # we replace any src= https://whatever.com  stripping the part up to the first slash
        content = re.sub(
            r"import\(\s*\s*(?P<quoteChar>['\"])https?://[^/\"']+/",
            r"import(\g<quoteChar>./",
            content,
            flags=re.IGNORECASE,
        )  # similar to above but for import('http://...  must add ./ as well
        proxyAdd = "<script blocking='render' src='JSNetProxy.js'></script>"

    content = validUntilFix(content)
    content = content.replace(
        "<head>", f"<head><script>{injectedjs}</script>{proxyAdd}"
    )
    content = content.replace(
        'from "https://static.matterport.com', 'from ".'
    )  # fix the direct code import they added
    ...
    for x in range(0, increment):
        for y in range(0, increment):
            ...
            if yPosStr.endswith(".0"):
                yPosStr = yPosStr[:-2]
            outFilename = (
                f"{jpgFilePath}crop={cropSize},{cropSize},x{xPosStr},y{yPosStr}.jpg"
            )
            xPos *= maxSize
            yPos *= maxSize
...
async def AdvancedAssetDownload(base_page_text: str):
    ...
    ADV_CROP_FETCH = [
        {"start": "width=512&crop=1024,1024,", "increment": "0.5"},
        {"start": "crop=512,512,", "increment": "0.25"},
    ]
    ...
    try:
        base_node: Any = (
            None  # lets try to use this now first seems to be more accurate the precache key can be invalid in comparison
        )
        base_cache_node: Any = None
        base_node_snapshots: Any = None
        try:
            with open(
                "api/mp/models/graph_GetModelDetails.json", "r", encoding="UTF-8"
            ) as f:
                graphModelDetailsJson = json.loads(f.read())
                base_node = graphModelDetailsJson["data"]["model"]
        except Exception:
            logging.exception(
                "Unable to open graph model details output json something probably wrong....."
            )

        try:
            with open(
                "api/mp/models/graph_GetSnapshots.json", "r", encoding="UTF-8"
            ) as f:
                graphModelSnapshotsJson = json.loads(f.read())
                base_node_snapshots = graphModelSnapshotsJson["data"]["model"]
        except Exception:
            logging.exception(
                "Unable to open graph model for snapshots output json something probably wrong....."
            )

        match = re.search(
            r"window.MP_PREFETCHED_MODELDATA = (\{.+?\}\}\});", base_page_text
        )
        preload_json_str = None
        if not match:
            match = re.search(
                r"window.MP_PREFETCHED_MODELDATA = parseJSON\((\"\{.+?\}\}\}\")\);",
                base_page_text,
            )  # this happens for extra unicode encoded pages
            consoleDebugLog(
                "Main page embedded preset data was unicode/parseJSON passed instead of normal"
            )
            if not match:
                logging.exception(
                    "Unable to open graph model for snapshots output json something probably wrong....."
                ...
                consoleLog(
                    "###### UNABLE to extract pre-fetch data from main page, will try to proceed but likely have issues",
                    logging.WARNING,
                )
            else:
                preload_json_str = json.loads(
                    match.group(1)
                )  # yes we load it here first, it is a string passed to parseJson so this basically unescapes that string into preload_json_str which will then be loaded again later
        else:
            preload_json_str = match.group(1)

        if preload_json_str is not None:
            preload_json = json.loads(
                preload_json_str
            )  # in theory this json should be similar to GetModelDetails, sometimes it is a bit different so we may want to switch
            base_cache_node = preload_json["queries"]["GetModelPrefetch"]["data"][
                "model"
            ]
        if CLA.getCommandLineArg(CommandLineArg.DEBUG):
            DebugSaveFile(
                "base_page_extracted_json.json", json.dumps(preload_json, indent="\t")
            )  # noqa: E701
            DebugSaveFile(
                "base_page_extracted_precache_advanced_model_data.json",
                json.dumps(base_cache_node, indent="\t"),
            )  # noqa: E701
            DebugSaveFile(
                "advanced_model_data_from_GetModelDetails.json",
                json.dumps(base_node, indent="\t"),
            )  # noqa: E701
            DebugSaveFile(
                "advanced_model_data_from_GetSnapshots.json",
                json.dumps(base_node_snapshots, indent="\t"),
            )  # noqa: E701

        if not base_cache_node:
            consoleLog(
                "No embedded cache node falling back to Graph GetModelDetails query",
                logging.WARNING,
            )
            base_cache_node = base_node
        if not base_node:
            consoleLog(
                "Not sure data query GetModelDetails worked we didn't get json back as expected, generally we don't need it for the dl run as we use embedded cache",
                logging.INFO,
            )
            base_node = base_cache_node
        if (
            "locations" not in base_node
        ):  # the query doesnt get locations back but the cache does have it, this is expected true
            base_node["locations"] = base_cache_node["locations"]

        if MODEL_IS_DEFURNISHED:
            KeyHandler.SetAccessKey(
                AccessKeyType.MAIN_PAGE_GENERIC_KEY, "INVALIDGENERICACCESSKEY"
            )
            KeyHandler.SetAccessKey(
                AccessKeyType.FILES2_BASE_URL_KEY, "INVALIDGENERICACCESSKEYFILE2"
            )
            KeyHandler.SetAccessKey(
                AccessKeyType.FILES3_TEMPLATE_KEY, "INVALIDGENERICACCESSKEYFILES3"
            )
            KeyHandler.SetAccessKey(
                AccessKeyType.MAIN_PAGE_DAM_50K, "INVALIDGENERICACCESSKEYDAM"
            )
            ...
            KeyHandler.SetAccessKey(
                AccessKeyType.SWEEP_KEY,
                KeyHandler.GetKeysFromStr(
                    base_cache_node["locations"][0]["pano"]["skyboxes"][0][
                        "tileUrlTemplate"
                    ]
                )[0],
            )
            ...
            KeyHandler.SetAccessKey(
                AccessKeyType.MAIN_PAGE_GENERIC_KEY,
                KeyHandler.GetKeysFromStr(
                    base_cache_node["assets"]["textures"][0]["urlTemplate"]
                )[0],
            )
            if (
                base_cache_node["defurnishViews"]
                and base_cache_node["defurnishViews"][0]
            ):
                consoleLog(
                    f"#### WARNING: Model has defurnished view if you want to download it you must run a second matterport-dl.py download on its id of: {base_cache_node['defurnishViews'][0]['model']['id']}",
                    loglevel=logging.WARN,
                )
        toDownload: list[AsyncDownloadItem] = []
        if base_node_snapshots:
            consoleDebugLog(
                f"AdvancedDownload photos: {len(base_node_snapshots['assets']['photos'])} meshes: {len(base_node['assets']['meshes'])}, locations: {len(base_node['locations'])}, tileset indexes: {len(base_node['assets']['tilesets'])}, textures: {len(base_node['assets']['textures'])}, "
            )
        ...
        for mesh in base_cache_node["assets"][
            "meshes"
        ]:  # generally there is 50k and 500k but 500k we dont seem to have access to
            ...
            toDownload.append(
                AsyncDownloadItem(
                    ...
                    "50k" not in mesh["url"],
                    mesh["url"],
                    urlparse(mesh["url"]).path[1:],
                    key_type=AccessKeyType.LeaveKeyAlone,
                )
            )  # not expecting the non 50k one to work but mgiht as well try
        ...
        for photo in base_node_snapshots["assets"]["photos"]:
            ...
            if not imageUrl:
                imageUrl = photo[
                    "presentationUrl"
                ]  # fallback not sure we every need this
            toDownload.append(
                AsyncDownloadItem(
                    "ADV_MODEL_IMAGES",
                    True,
                    imageUrl,
                    urlparse(imageUrl).path[1:],
                    key_type=AccessKeyType.LeaveKeyAlone,
                )
            )
        ...
        do4K = (
            len(base_node["locations"]) == 0
        )  # by default only do 4k if we have no locations data to check
        for location in base_node["locations"]:
            if "4k" in location["pano"]["resolutions"]:
                ...
                if CLA.getCommandLineArg(CommandLineArg.DEBUG):
                    raise Exception(
                        "Found a non 4k pano even though others are 4k, dynamic resolution detection needed"
                    )
            for skybox in location["pano"]["skyboxes"]:
                if (
                    skybox["status"] == "locked"
                ):  # not sure why some models have 4k but then locked skyboxes at that resolution.  The normal skybox key also with 403  trying to access the 4ks.  We can use the main key to access the urls but it just gives a 404
                ...
                try:
                    for face in range(6):
                        skyboxUrlTemplate = skybox["urlTemplate"].replace(
                            "<face>", f"{face}"
                        )
                        toDownload.append(
                            AsyncDownloadItem(
                                ...
                                False,
                                skyboxUrlTemplate,
                                urlparse(skyboxUrlTemplate).path[1:],
                                key_type=AccessKeyType.LeaveKeyAlone,
                            )
                        )
                except:
        ...
        for tileset in base_cache_node["assets"][
            ...
            try:
                ...
                uris = re.findall(
                    r'"uri":"(.+?)"', tileSetText
                )  # a bit brutish to extract rather than just walking the json
                ...
                for uri in tqdm(uris):
                    ...
                    try:
                        chunkBytes = await downloadFileAndGetText(
                            "ADV_TILESET_GLB",
                            False,
                            url,
                            urlparse(url).path[1:],
                        ...
                        chunks = re.findall(
                            r"(lod[0-9]_[a-zA-Z0-9-_]+\.(jpg|ktx2))", chunkText
                        )
            ...
            for file in range(tilesetDepth + 1):
                try:
                    tileseUrlTemplate = tilesetUrlTemplate.replace(
                        "<file>", f"{file}.json"
                    )
                    getFileText = await downloadFileAndGetText(
                        "ADV_TILESET_JSON",
                        False,
                        tileseUrlTemplate,
                        urlparse(tileseUrlTemplate).path[1:],
                        key_type=AccessKeyType.LeaveKeyAlone,
                    )
                ...
                for i in range(1000):
                    ...
                    for crop in crop_to_do:
                        for x in list(drange(0, 1, decimal.Decimal(crop["increment"]))):
                            for y in list(
                                drange(0, 1, decimal.Decimal(crop["increment"]))
                            ):
                                xs = f"{x}"
                                ys = f"{y}"
                                if xs.endswith(".0"):
                                ...
                                toDownload.append(
                                    AsyncDownloadItem(
                                        "ADV_TEXTURE_CROPPED",
                                        False,
                                        full_text_url + "&" + complete_add,
                                        urlparse(full_text_url).path[1:]
                                        + complete_add_file
                                        + ".jpg",
                                    )
                                )  # failures here ok we dont know all teh crops that exist, so we can still use the array downloader
...
def getPageId(url):
    ...
    if not id.isalnum() or (
        (len(id) < 5 or len(id) > 15) and not MODEL_IS_DEFURNISHED
    ):  # 25 can be used for defurnished models
        raise Exception(
            f"Likely invalid model id extracted: {id} from your input of: {url} you should pass the ID itself (ie EGxFGTFyC9N) or the url: form like: https://my.matterport.com/show/?m=EGxFGTFyC9N"
        )
    return id
...
class OurSimpleHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not os.path.exists(self.path[1:]) and "~" in self.path:
            alt_path = self.path.replace("~", "_")
            if os.path.exists(alt_path[1:]): # [1:] strips the leading slash for os.path
                self.path = alt_path
                redirect_msg = "Transparently serving '_' version for '~' request"
        ...
        if raw_path.startswith("/JSNetProxy.js"):
            consoleDebugLog(
                "Using our javascript network proxier", loglevel=logging.INFO
            )
            self.send_response(200)
            self.end_headers()
            with open(
                os.path.join(BASE_MATTERPORTDL_DIR, "JSNetProxy.js"),
                "r",
                encoding="UTF-8",
            ) as f:
        ...
        if raw_path.startswith("/locale/messages/strings_") and not os.path.exists(
            f".{raw_path}"
        ):
        ...
        if "crop=" in query and raw_path.endswith(".jpg"):
            ...
            if os.path.exists(f".{test_path}"):
                ...
                consoleDebugLog(
                    f"Requested crop texture not found: {test_path} falling back to full res may show visual issues",
                    loglevel=logging.WARNING,
                )
    ...
    def do_GraphRequest(self, option_name: str):
        ...
        if post_msg is not None:
            consoleDebugLog(
                f"Handling a graph request on {self.path}: {post_msg}",
                loglevel=logLevel,
            )
...
def SetupSession(use_proxy):
    ...
    OUR_SESSION = requests.AsyncSession(
        impersonate="chrome",
        max_clients=MAX_CONCURRENT_REQUESTS,
        verify=CLA.getCommandLineArg(CommandLineArg.VERIFY_SSL),
        proxies=({"http": use_proxy, "https": use_proxy} if use_proxy else None),
        headers={
            "Referer": f"https://my.{BASE_MATTERPORT_DOMAIN}/",
...
def startServer(baseDir, pageId, browserLaunch, bindAddress, bindPort):
    ...
    if not os.path.exists(twinDir):
        ...
        if os.path.exists(relativeToScriptDir):
            ...
            raise Exception(
                f"Unable to change to download directory for twin of: {fullPath} or {os.path.abspath(relativeToScriptDir)} make sure the download is there"
            )
    else:
        os.chdir(twinDir)
    try:
...
class KeyHandler:
    ...
    RE_ACCESS_KEY_EXTRACT: ClassVar[re.Pattern] = re.compile(
        r"2\-[0-9a-z]{40}\-17[0-9]{8}-[0-9]"
    )  # important to remain case sensitive
    ...
    def SetAccessKey(key_type: AccessKeyType, key: str):
        if type(key) is not str or not key:
            raise Exception(
                f"Call with invalid key for SetAccessKey {key_type} = {key}"
            )
        ...
        if (
            KeyHandler.PrimaryKey is None
            and key_type == AccessKeyType.FILES3_TEMPLATE_KEY
        ):  # our former primary key
            KeyHandler.PrimaryKey = key
    ...
    async def PrintUrlKeys(url, isDownload):
        ...
        async with aiofiles.tempfile.TemporaryDirectory() as tmp:
            ...
            debugTargetName = os.path.join(
                "debug", os.path.basename(urlparse(url).path[1:])
            )
            for key in KeyHandler.KNOWN_ACCESS_KEYS:
                toDownload.append(
                    AsyncDownloadItem(
                        "FindUrlKey",
                        False,
                        KeyHandler.SetAccessKeyForUrl(url, key),
                        os.path.join(tmp, "test_" + key),
                        key_type=AccessKeyType.LeaveKeyAlone,
                    )
                )
            await AsyncArrayDownload(toDownload)
            for _file in os.listdir(tmp):
                ...
                file = _file[5:]
                workingKeys += (
                    f"\t{file}({KeyHandler.KNOWN_ACCESS_KEYS[file].strip()})\n"
                )
...
CommandLineArg = Enum(
    ...
    [
        ...
        "DOWNLOAD",
        "MAIN_ASSET_DOWNLOAD",
        "MANUAL_HOST_REPLACEMENT",
        "ALWAYS_DOWNLOAD_GRAPH_REQS",
        "QUIET",
        "HELP",
        "ADV_HELP",
        "AUTO_SERVE",
        "FIND_URL_KEY",
        "FIND_URL_KEY_AND_DOWNLOAD",
        "REFRESH_KEY_FILES",
        "GENERATE_TILE_MESH_CROPS",
        "TITLE",
        "ENUM_RANGES",
    ],
)
...
class CLA:
    ...
    orig_args: ClassVar[list[str]] = (
        []
    )  # we store them so we can reparse them after a config load
    value_cache: ClassVar[dict[CommandLineArg, Any]] = {}  # faster lookup
    enum_ranges: str = ""

    @staticmethod
    def addCommandLineArg(
        arg: CommandLineArg,
        description: str,
        defaultValue: Any,
        itemValueHelpDisplay: str = "",
        hidden=False,
        allow_saved=True,
        applies_to=ArgAppliesTo.DOWNLOAD,
    ):
        ...
        cla = CLA(
            ...
            hasValue=itemValueHelpDisplay != "",
            itemValueHelpDisplay=itemValueHelpDisplay,
            hidden=hidden,
            allow_saving=allow_saved,
        ...
        for i in range(1, len(CLA.orig_args)):
            for cla in CLA.all_args:
                ...
                if CLA.orig_args[i] == f"--{cla.argConsoleName()}" or isNegativeName:
                    if cla.hasValue:
                        cla.currentValue = (
                            CLA.orig_args[i + 1] if not isNegativeName else ""
                        )
                    else:
...
def main():
    CLA.addCommandLineArg(
        CommandLineArg.BASE_FOLDER,
        "folder to store downloaded models in (or serve from)",
        "./downloads",
        itemValueHelpDisplay="dir",
        allow_saved=False,
        applies_to=ArgAppliesTo.BOTH,
    )
    CLA.addCommandLineArg(
        CommandLineArg.PROXY,
        "using web proxy specified for all requests, use --no-verify-ssl to disable ssl verification",
        "",
        "127.0.0.1:8866",
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.TILDE,
        "allowing tildes on file paths, likely must be disabled for Apple/Linux, you must use the same option during the capture and serving",
        sys.platform == "win32",
    )
    CLA.addCommandLineArg(
        CommandLineArg.ALIAS,
        "create an alias symlink for the download with this name, does not override any existing (can be used when serving)",
        "",
        itemValueHelpDisplay="name",
    )
    CLA.addCommandLineArg(
        CommandLineArg.ADVANCED_DOWNLOAD,
        "downloading advanced assets enables things like skyboxes, dollhouse, floorplan layouts, now primary access keys come from it so generally required",
        True,
    )
    CLA.addCommandLineArg(
        CommandLineArg.DEBUG,
        "debug mode enables select debug output to console or the debug/ folder mostly for developers",
        False,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.CONSOLE_LOG,
        "showing all log messages in the console rather than just the log file, very spammy",
        False,
        allow_saved=False,
    )

    CLA.addCommandLineArg(
        CommandLineArg.DOWNLOAD,
        "Download items (without this it just does post download actions)",
        True,
        hidden=True,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.VERIFY_SSL,
        "SSL verification, mostly useful for proxy situations",
        True,
        allow_saved=False,
        hidden=True,
    )
    CLA.addCommandLineArg(
        CommandLineArg.MAIN_ASSET_DOWNLOAD,
        "Primary asset downloads (normally biggest part of the download)",
        True,
        hidden=True,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.ALWAYS_DOWNLOAD_GRAPH_REQS,
        "Always download/make graphql requests, a good idea as they have important keys, note if REFRESH_KEY_FILES is off it will still prevent graph files from downloading",
        True,
        hidden=True,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.FIND_URL_KEY,
        "A URL to try to find the access key for, makes a few minimal requests upfront to get needed keys",
        "",
        "https://my.matterport.com/api/player/models/EGxFGTFyC9N/test.file",
        hidden=True,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.FIND_URL_KEY_AND_DOWNLOAD,
        "Like FIND_URL_KEY but saves a copy to the debug folder of the item",
        "",
        "https://my.matterport.com/api/player/models/EGxFGTFyC9N/test.file",
        hidden=True,
        allow_saved=False,
    ...
    CLA.addCommandLineArg(
        CommandLineArg.REFRESH_KEY_FILES,
        "There are about a half dozen files always downloaded as they may contain access keys we need, this prevents these from downloading",
        True,
        hidden=True,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.GENERATE_TILE_MESH_CROPS,
        "Certain views like dollhouse require cropped versions of certain textures, this uses python to generate all those",
        True,
        hidden=False,
        allow_saved=True,
    )
    ...
    CLA.addCommandLineArg(
        CommandLineArg.MANUAL_HOST_REPLACEMENT,
        "Use old style replacement of matterport URLs rather than the JS proxy, this likely only works if hosted on port 8080 after",
        False,
        hidden=True,
    )

    CLA.addCommandLineArg(
        CommandLineArg.QUIET,
        "Only show failure log message items when serving",
        False,
        applies_to=ArgAppliesTo.SERVING,
        allow_saved=False,
    )
    CLA.addCommandLineArg(
        CommandLineArg.AUTO_SERVE,
        "Used to automatically start the server hosting a specific file, see README for details",
        "",
        "page_id_or_alias|host|port|what-browser",
        applies_to=ArgAppliesTo.SERVING,
        hidden=True,
    )

    CLA.addCommandLineArg(
        CommandLineArg.HELP, "", False, hidden=True, allow_saved=False
    )
    CLA.addCommandLineArg(
        CommandLineArg.TITLE,
        "Model title override, normally extracted from page title",
        "",
        hidden=True,
    )
    CLA.addCommandLineArg(
        CommandLineArg.ADV_HELP,
        "Show advanced command line options normally hidden, not recommended for most users",
        False,
        hidden=False,
        allow_saved=False,
        applies_to=ArgAppliesTo.BOTH,
    ...
    CLA.addCommandLineArg(
        CommandLineArg.ENUM_RANGES,
        "ranges for targeted chunk enumeration (e.g. 200-300,2000-3000)",
        "",
        itemValueHelpDisplay="ranges",
    )
    CLA.parseArgs()
    CLA.enum_ranges = CLA.getCommandLineArg(CommandLineArg.ENUM_RANGES)
    browserLaunch = ""
    ...
    subProcessArgs = (
        CLA.orig_args.copy()
    )  # args we give the interactive UI minus ip and port as it only uses them for downloads
    if len(sys.argv) > argPos:
        ...
        if not ip_pattern.match(pageIdOrIp):
            ...
            if len(sys.argv) == argPos:  # if no more args its a download run
                isDownloadRun = True
        if (
            len(sys.argv) == argPos + 2
        ):  # ip and port left, note if it was an IP above we didn't increment argPos
            isServerRun = pageId != ""
    ...
    if (
        not os.path.exists(os.path.join(baseDir, pageId))
        and os.path.exists(pageId)
        and isServerRun
    ):  # allow old rooted pages to still be served
        baseDir = "./"
    ...
    if os.path.exists(existingConfigFile):
        try:
            ...
            pass
    isExplicitHelpCLI = CLA.getCommandLineArg(
        CommandLineArg.HELP
    ) or CLA.getCommandLineArg(CommandLineArg.ADV_HELP)
    if isExplicitHelpCLI or (not isServerRun and not isDownloadRun):
        ...
        if not isExplicitHelpCLI and sys.stdin.isatty():
            try:
                ...
                from _matterport_interactive import (
                    interactiveManagerGetToServe,
                    print_colored,
                    bcolors,
                )

                print(
                    "Running in interactive mode.\n\tIf you instead wanted command line usage start this script with: ",
                    end="",
                )
                ...
                print(
                    "Error: Could not import interactive start from _matterport_interactive.py"
                )
    ...
    if isDownloadRun:
        ...
        if not CLA.getCommandLineArg(CommandLineArg.TILDE):
            consoleLog("Applying final underscore compatibility patches...")
            model_dir = os.path.join(baseDir, pageId)
                    ...
                    if name.endswith((".html", ".js", ".json")):
                        ...
                        try:
                            ...
                            new_content = content.replace(
                                "runtime~showcase", "runtime_showcase"
                            )
                            new_content = new_content.replace(
                                "%7E",  "_"
                            )
                            new_content = new_content.replace(
                                "mesh_tiles/~/", "mesh_tiles/_/"
                            )
                            new_content = new_content.replace(
                                "assets/~/tiles/", "assets/_/tiles/"
                            )
                            ...
                            consoleLog(f"Patch error in {name}: {e}")
